...
/**
 * The real optimization engine. Every byte count comes from actual file sizes.
 * A file is only replaced when the optimized version is genuinely smaller.
 */
class OptiPress_Processor {
	...
	/**
	 * Optimize one item (full size + all registered sizes), then optionally convert.
	 *
	 * @param int   $item_id
	 * @param array $opts {origin:auto|manual|bulk, force:bool, webp:bool, avif:bool}
	 * @return array Result summary for UI/AJAX.
	 */
	public function process( $item_id, $opts = array() ) {
		...
		// Guard: don't double-process an image another operation already claimed
		// (e.g. auto-optimize cron firing while a bulk run is active).
		...
		if ( ! $item->file || ! file_exists( $abs ) ) {
			$this->mark_failed( $item, 'missing_file', __( 'The file no longer exists on disk. It may have been moved or deleted outside WordPress.', 'optipress' ) );
		...
		if ( ! is_readable( $abs ) || ! is_writable( $abs ) ) {
			$this->mark_failed( $item, 'permissions', __( 'File permissions prevent reading or writing this image. Fix file permissions (usually 644) and retry.', 'optipress' ) );
		...
		if ( 'none' === $caps['engine'] ) {
			$this->mark_failed( $item, 'no_engine', __( 'No image processing library (Imagick or GD) is available on this server.', 'optipress' ) );
		...
		if ( filesize( $abs ) > $max_bytes ) {
			$msg = sprintf(
				...
				__( 'Skipped: file is larger than the configured limit (%1$s > %2$s). Raise the limit in Settings → General to process it.', 'optipress' ),
		...
		// Backup originals once, before any destructive write.
		...
		// Raise memory headroom for large originals.
		$orig_total = 0;
		...
		foreach ( $files as $key => $path ) {
			...
			if ( $r['ok'] ) {
				...
				if ( 'skipped' === $r['code'] ) {
					...
					$new_total += $r['orig']; // Unchanged file keeps its size.
				}
			}
		}

		// All hard-failed → item failed. Some success → optimized with warnings.
		...
		// Regenerate conversions that already existed (re-optimize flow).
		...
		return array(
			...
			'pct'     => $pct,
			'message' => sprintf(
				__( 'Saved %1$s (%2$s%%).', 'optipress' ),
				size_format( $saved ), number_format_i18n( $pct, 1 )
			),
			'warnings' => array_values( $errors ),
			'notes'    => $notes,
		);
	...
	/**
	 * Optimize a single file. Replaces it only if the result is smaller.
	 *
	 * @return array {ok,code,message,orig,new,saved,note?}
	 */
	private function optimize_file( $path, $mode, $quality, $strip, $caps ) {
		...
		if ( ! $info ) {
			return array( 'ok' => false, 'code' => 'corrupt', 'message' => __( 'This file is not a valid image (corrupt or truncated).', 'optipress' ), 'orig' => $orig, 'new' => $orig, 'saved' => 0 );
		}
		$mime = $info['mime'];

		// Animated GIFs cannot be safely re-encoded by GD/Imagick without frame loss.
	...
	private function encode_imagick( $path, $tmp, $mime, $mode, $quality, $strip ) {
		...
		try {
			...
			if ( $strip ) {
				try { $im->stripImage(); } catch ( \Throwable $e ) { /* non-fatal */ }
			}
	...
	/** Cheap photographic heuristic for PNG decisions (high color count / large area). */
	private function looks_photographic( $path ) {
		$info = @getimagesize( $path );
		if ( ! $info ) { return false; }
		return ( $info[0] * $info[1] ) > 500000; // > ~0.5MP
	}

	private function is_animated_gif( $path ) {
		$fh = @fopen( $path, 'rb' );
		if ( ! $fh ) { return false; }
		$data = fread( $fh, 1024 * 1024 ); // Scan first 1MB.
		fclose( $fh );
	...
	/** Translate raw encoder errors into human language. */
		...
		switch ( $code ) {
			case 'memory':
				return sprintf(
					...
					__( 'Optimization failed because the server ran out of memory while processing this %s image. Ask your host to raise the PHP memory limit, or lower the “Skip files larger than” setting.', 'optipress' ),
					size_format( $orig_bytes )
				);
			case 'permissions':
				return __( 'Optimization failed due to a file permission problem. The web server cannot write to this file or its folder.', 'optipress' );
			case 'disk_full':
				return __( 'Optimization failed because the server has insufficient disk space for temporary files.', 'optipress' );
			case 'timeout':
				return __( 'Optimization failed because processing exceeded the server time limit. Try a smaller batch size in Settings → Bulk.', 'optipress' );
			default:
				return __( 'The image could not be decoded — the file appears to be corrupt or uses an unsupported encoding.', 'optipress' ) . ' (' . $e->getMessage() . ')';
		}
	}
